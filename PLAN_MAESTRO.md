...
| Sistema MTF SL/TP + Fibonacci | 🟢 | ⚪ no verificado funcionalmente | `core/risk/mtf_sl_tp_manager.py` (810 líneas — god file, ver auditoría de calidad) |
...
| Frontend (7 páginas Streamlit + dashboard HTML/JS nativo) | 🟢 | ⚪ no verificado en navegador | `app/pages/*.py` (7), dashboard servido desde FastAPI |
...
|---|-------|--------|--------------------|
| 1 | Token JWT sin módulo dedicado de blacklist (`core/auth/token_blacklist.py` no existe como archivo) | 🟡 la lógica sí existe y funciona dentro de `jwt_handler.py`, pero se salta silenciosamente si Redis cae (severidad baja) | 6h (extraer a módulo propio + endurecer fallo) |
| 2 | Cobertura de tests en `api/routes/` | 🔴 confirmado insuficiente — 35 tests fallando, `test_api_execution.py` no colecta | 16h+ |
| 3 | `dashboard.py` God File (931 líneas en plan de mayo) | ⚪ no re-verificado en esta pasada; nuevos god files confirmados: `mtf_sl_tp_manager.py` (810L), `i1_gate_validator.py` (730L), `asset_specific_models.py` (745L) | 8h + 24h para los nuevos |
...
3. **`data/processed/`** (features cacheados) sigue sin existir — entregable declarado desde abril, nunca generado. Sin cambios.
4. **`app/components/`** sigue sin existir. Sin cambios.
5. **`core/auth/token_blacklist.py`** no existe como archivo separado, pero se confirmó que la lógica de blacklist sí funciona dentro de `jwt_handler.py` (gap es de organización/documentación, no funcional — salvo por el fallo silencioso si Redis cae).
...
7. **Cerrar gaps de arquitectura:** unificar la interfaz de exchange (elegir entre `ExchangeClient`/`ExchangeAdapter`), deduplicar ATR, refactorizar los god files confirmados (`mtf_sl_tp_manager.py`, `i1_gate_validator.py`, `asset_specific_models.py`).
8. **Cerrar gaps baratos restantes:** extraer `token_blacklist.py` a módulo propio, decidir sobre `app/components/` y `data/processed/`.
