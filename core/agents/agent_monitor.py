...
class AgentMonitor:
    ...
    def __init__(self, window_size: int = 100):
        ...
        self.predictions: dict[str, deque] = defaultdict(
            lambda: deque(maxlen=window_size)
        )
        self.outcomes: dict[str, deque] = defaultdict(lambda: deque(maxlen=window_size))
        self.confidence_scores: dict[str, deque] = defaultdict(
            lambda: deque(maxlen=window_size)
        )
    ...
    def record_outcome(
        self, agent_id: str, symbol: str, actual_direction: str, pnl: float
    ) -> None:
    ...
    def get_performance_report(self, agent_id: str) -> dict:
        ...
        for outcome in outcomes:
            ...
            pred = next(
                (
                    ...
                    if p["symbol"] == outcome["symbol"]
                    and p["timestamp"] < outcome["timestamp"]
                ),
                None,
            )
            if pred:
        ...
        return {
            "agent_id": agent_id,
            "period_start": (
                predictions[0]["timestamp"].isoformat() if predictions else None
            ),
            "period_end": (
                predictions[-1]["timestamp"].isoformat() if predictions else None
            ),
