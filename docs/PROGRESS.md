...
|---|---|---|---|---|
| F0 — Gobernanza mínima | 2 | 0 | En curso | — |
| F1 — Habilitación mínima del core | 9 | 0 | Pendiente | — |
...
|---|---|---|---|---|
| 0.1 | ADR-003 (core = agentes + consenso; estrategias = params I1), ADR-004 (universo canónico + nomenclatura), ADR-005 (kill switch fail-safe; blacklist fail-open) | Hecho (pend. auditoría) | `docs/adr/ADR-003-*.md`, `docs/adr/ADR-004-*.md`, `docs/adr/ADR-005-*.md` | — |
| 0.2 | `docs/audits/AUDIT_TEMPLATE.md` + `docs/audits/BASELINE_2026-08-28.md` (21 áreas, congelado) + `docs/PROGRESS.md` (una fila por entregable) | Hecho (pend. auditoría) | `docs/audits/AUDIT_TEMPLATE.md`, `docs/audits/BASELINE_2026-08-28.md`, `docs/PROGRESS.md` | — |

**Auditoría de cierre F0:** las 3 ADRs con secciones "Decisión" y "Consecuencias", sin contradecir el código; `BASELINE` con score por área. → `docs/audits/AUDIT_F0_<fecha>.md`

### F1 — Habilitación mínima del core

| ID | Entregable | Spec | Cierra | Estado | Evidencia | PR |
|---|---|---|---|---|---|---|
| 1.1 | Split worker/API: quitar APScheduler + `_refresh_task` del `lifespan`; `run_pipeline.py` único dueño con lock Redis `SET NX EX`; servicio `worker` en compose | SPEC-A02 | F-04, D-04 | Pendiente | | |
| 1.2 | Empaquetado + persistencia: `COPY alembic/` + `alembic.ini`; fail-fast si `EXECUTION_MODE=live` sin DB; `/health` expone `db_initialized`; HEALTHCHECK sin `requests` | SPEC-A03 | F-05, D-05, D-22 | Pendiente | | |
| 1.3 | Un universo de activos: `TRADED_UNIVERSE` único en `core/config/constants.py` + dict de mapeo a broker; los 3 consumidores lo importan; símbolos sin datos → skip con log | SPEC-B03 | G-10, D-11 | Pendiente | | |
| 1.4 | Una fuente de estrategias + conectar params I1 al pipeline: `StrategyRegistry` catálogo único; gate I1 escribe `data/models/i1_params/<symbol>.json`; pipeline carga params por activo; sin archivo aprobado ⇒ no emite señales; `grep default_v1` → 1 punto | SPEC-B06, SPEC-D01 | F-03, G-3 | Pendiente | | |
| 1.5 | Correctitud de decisiones: fix tests HMM; exposición agregada de cartera en `RiskManager.validate_signal`; `run_pipeline` pasa `recent_trades` reales a `update_kill_switch` | SPEC-A08, SPEC-A05 (f1) | D-12, F-09, D-09 | Pendiente | | |
| 1.6 | Trazabilidad mínima + productor: `correlation_id` en `_pipeline_cycle` propagado y persistido; `GET /trace/{id}`; pipeline persiste señal vía repo + registra decisión en `AuditLog` | SPEC-B04, F-06 | F-06, G-6 | Pendiente | | |
| 1.7 | Suelo de seguridad: `REGISTRATION_ENABLED` (default false); registro público solo `viewer`; `POST /auth/users` con `require_admin`; validador de `JWT_SECRET_KEY` siempre activo | SPEC-A01 (subset) | F-01, F-08 | Pendiente | | |
